...
def _colorize_density_maps(
    density_maps: tf.Tensor, *, max_density_threshold: float
) -> tf.Tensor:
    """
    Converts density maps to false-color to make them easier to view by humans.

    Args:
        density_maps: The 4D tensor of density maps.
    ...
    Returns:
        The colorized density maps. Note that these will be returned as uint8
        images, ready for display.

    """
    # Low values should show up as blue.
    blue_channel = max_density_threshold - density_maps
    # High values should show up as yellow.
    red_channel = density_maps
    green_channel = density_maps
    ...
    color_maps = color_maps / max_density_threshold * tf.constant(255.0)
...
def visualize_density_maps(
    *,
    ...
    """
    Generates a nice visualization for density maps that overlays the map
    on the actual image.

    Args:
        images: The raw input images, as a 4D tensor.
        density_maps: The corresponding density maps, as a 4D tensor.
        max_density_threshold: Density threshold to use for colorization.
            Any pixel with this density or more will show up as the maximum
            density color.

    Returns:
        A 4D tensor containing density map visualizations.

    """
    # Get the colorized density maps.
    colored_maps = _colorize_density_maps(
        density_maps, max_density_threshold=max_density_threshold
    )
