...
class BaseModel(nn.Module):
    ...
    def __init__(
        self,
        *args,
        embedding_layer: int = 5,
        output_raw_embeddings: bool = False,
        **kwargs,
    ):
        """
        Args:
            *args: Will be forwarded to the superclass.
            ...
            output_raw_embeddings: If true, it will return both embeddings
                and the model output from every prediction run, and the
                embeddings will be returned without average pooling.
            **kwargs: Will be forwarded to the superclass.

        """
        self.embedding_layer = embedding_layer
        self.output_raw_embeddings = output_raw_embeddings
    ...
    def predict(self, x, profile=False, visualize=False, augment=False, embed=None):
        ...
        return self._predict_once(
            x, profile, visualize, embed, raw_embeddings=output_raw
        )

    def _predict_once(
        self, x, profile=False, visualize=False, embed=None, raw_embeddings=False
    ):
        """
        Perform a forward pass through the network.
        ...
        Args:
            ...
            visualize (bool): Save the feature maps of the model if True, defaults to False.
            embed (list, optional): A list of feature vectors/embeddings to return.
            raw_embeddings (bool, optional): If true, will output the raw
                embeddings along with the normal output. (embed must be set)
        ...
        for m in self.model:
            ...
            if embed and m.i in embed:
                ...
                if not raw_embeddings and m.i == max(embed):
